...
void baseten_function_deployments(
    ...
    void *data __maybe_unused)
{
    ...
    collector_info("BASETEN: Function called (transaction: %s, info: %s)", transaction, info ? "yes" : "no");
    ...
    if (info) {
        // Info request - add protocol version and accepted parameters
        collector_info("BASETEN: Returning function metadata (info request)");

        // Protocol version 3 indicates POST should be used for data requests
    ...
    while (deployment) {
        ...
        buffer_json_add_array_item_string(
            wb,
            deployment->model && deployment->model->instance_type_name ? deployment->model->instance_type_name :
                                                                         "Unknown");
    ...
    buffer_json_member_add_object(wb, "columns");
    {
        ...
        buffer_rrdf_table_add_field(
            ...
            NULL,
            NAN,
            RRDF_FIELD_SORT_ASCENDING | RRDF_FIELD_SORT_FIXED,
            NULL,
